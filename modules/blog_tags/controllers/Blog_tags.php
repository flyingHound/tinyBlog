<?php
class Blog_tags extends Trongate {
    ...
    public function create(): void {
        ...
        if ($update_id > 0) {
            $data['headline'] = 'Update Tag Record';
            $data['cancel_url'] = BASE_URL.'blog_tags/show/'.$update_id;
        } else {
            $data['headline'] = 'Create New Tag Record';
            $data['cancel_url'] = BASE_URL.'blog_tags/manage';
        }

        $data['form_location'] = BASE_URL.'blog_tags/submit/'.$update_id;
    ...
    public function manage(): void {
        ...
        if (segment(4) !== '') {
            ...
            $data['headline'] = 'Manage Blog Tags';
        ...
        $pagination_data['pagination_root'] = 'blog_tags/manage';
        $pagination_data['record_name_plural'] = 'blog tags';
        ...
        if ($submit === 'Submit') {
            ...
            if ($result === true) {
                ...
                if ($update_id>0) {
                    ...
                    $this->model->update($update_id, $data, 'blog_tags');
                    $flash_msg = 'The record was successfully updated';
                } else {
                    //insert the new record
                    $update_id = $this->model->insert($data, 'blog_tags');
                    $flash_msg = 'The record was successfully created';
                }

                set_flashdata($flash_msg);
                redirect('blog_tags/show/'.$update_id);
    ...
    public function submit_delete(): void {
        ...
        if (($submit === 'Yes - Delete Now') && ($params['update_id']>0)) {
            ...
            $params['module'] = 'blog_tags';
            $this->model->query_bind($sql, $params);

            //delete the record
            $this->model->delete($params['update_id'], 'blog_tags');
    ...
    private function get_data_from_db(int $update_id): ?array {
        $record_obj = $this->model->get_where($update_id, 'blog_tags');
